...
## RC2 Pin and Local Consumption
- Pin file confirms RC2: `requirements/m_cache_shared_external.txt`
...
1. `cat requirements/m_cache_shared_external.txt` -> pass (`v0.1.0-rc2` confirmed)
...
3. `M_CACHE_SHARED_SOURCE=local pytest -q tests/test_m_cache_shared_shim.py::test_shim_local_mode_bypasses_external_import` -> pass (1/1)
...
Issues fixed by RC2 pin/install normalization:
- RC pin mismatch removed (`B001_PIN_MISMATCH_RC2_NOT_PINNED` no longer applies).
- Local sibling editable install is now workable in active repo venv.
- Facade mode checks (`external`, `local`, `auto`) pass.

Remaining true news-local blockers after normalization:
- `B002_VALIDATION_FAILURE`
- `B006_CLI_API_SEMANTIC_DRIFT`

Observed failure signature across remaining failures:
- `TypeError: load_json_schema() got an unexpected keyword argument 'project_root'`
- Affects augmentation contract validation and augmentation CLI submit/status flows in this repo.
